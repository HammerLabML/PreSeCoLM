...
class MyDataset(CustomDataset):
    ...
    def __init__(self, local_dir: str = None):
        ...
        print("Loading custom dataset from:", local_dir)
        self.load(local_dir)
        self.prepare()
    ...
    def _set_split(self, df, split):
        ...
        labels = df[['offensiveYN', 'intentYN', 'sexYN']].to_numpy()
        ...
        for i, label in enumerate(self.group_names):
            self.protected_groups[split][:, i] = df['mergedMinority'].apply(
                lambda x: 1 if label in str(x).lower() else 0
            ).astype('float64')
    ...
    def load(self, local_dir=None):
        ...
        # You can define splits here manually if needed, e.g., 80/10/10
