...
def plot_feature_histogram(X, y, labels=None, features=None, xlabel=None, ylabel=None, savefile=None):
    def plot_hist_to_axes(X, y, ax, feature, labels=None):
        miny = int(np.min(y))
        maxy = int(np.max(y))
        if miny < 0 or maxy > 1:
    ...
    if len(y.shape) == 1:
        if len(X.shape) == 1 or X.shape[1] == 1:
            fig, ax = plt.subplots()
            plot_hist_to_axes(X, y, ax, features[0], labels)
        else:
            fig, axes = plt.subplots(1,X.shape[1], figsize=(4*X.shape[1],6))       
            for feature_id in range(X.shape[1]):
                plot_hist_to_axes(X[:,feature_id], y, axes[feature_id], features[feature_id], labels)
    else:
        if y.shape[1] == 2 and len(labels) == 1:
            fig, axes = plt.subplots(1, X.shape[1], figsize=(4*X.shape[1],4*len(labels)))
            for feature_id in range(X.shape[1]):
                plot_hist_to_axes(X[:,feature_id], y[:,1], axes[feature_id], features[feature_id], [labels[0]])
        else:
            # multiple binary identity labels
            fig, axes = plt.subplots(y.shape[1], X.shape[1], figsize=(4*X.shape[1],4*len(labels)))
            for i, group in enumerate(labels):
                for feature_id in range(X.shape[1]):
                    plot_hist_to_axes(X[:,feature_id], y[:,i], axes[i][feature_id], features[feature_id], [group])
